...
decl_storage! {
    trait Store for Module<T: Trait> as Example {
        ...
        Prices get(fn prices): VecDeque<u32>; // Vec<u8> 是价格
    }
}
...
impl<T: Trait> Module<T> {
    ...
    fn fetch_price_from_coincap_assets_info(dot_price_par: &mut u32) -> Result<(), Error<T>> {
        ...
        if let Some(Some(dot_price)) = s_info.get::<DotPrice>() {
            ...
            *dot_price_par =
            Self::vec_to_u32(dot_price.data.priceUsd).map_err(|_| Error::<T>::ParseError)?;
            return Ok(());
        }
        ...
        if let Ok(_guard) = lock.try_lock() {
            match Self::fetch_n_parse() {
                Ok(gh_info) => {
                    s_info.set(&gh_info);
                    *dot_price_par = Self::vec_to_u32(gh_info.data.priceUsd)
    ...
    fn offchain_signed_tx(dot_price: u32) -> Result<(), Error<T>> {
        ...
        let result = signer.send_signed_transaction(|_acct|
			...
			Call::submit_price_signed(dot_price));
    ...
    fn offchain_unsigned_tx(
        /*block_number: T::BlockNumber*/ dot_price: u32,
    ) -> Result<(), Error<T>> {
        // let number: u32 = block_number.try_into().unwrap_or(0).try_into().unwrap();
        let call = Call::submit_price_unsigned(dot_price);
