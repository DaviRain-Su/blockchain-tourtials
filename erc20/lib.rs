...
pub trait FetchErc20 {
    ...
    fn create(owner: AccountId, total_supply: Balance);
...
mod erc20 {
    ...
    impl Erc20 {
        ...
        pub fn new(total_supply: Balance) -> Self {
            ...
            // }
            let _ = Self::env().extension().create(caller, total_supply);
        ...
        pub fn total_supply(&self) -> Result<Balance> {
            // self.total_supply
            let balance = Self::env().extension().total_supply()?;
            Ok(balance)
        }
        ...
        // 定义的获得某个账户有多少余额的函数
        #[ink(message)]
        pub fn balance_of(&self, account: AccountId) -> Result<Balance> {
            // *self.balances.get(&account).unwrap_or(&0)
            let balance = Self::env().extension().balance_of(account)?;
            Ok(balance)
        }

        // 定义的获得owner可以向spend转账的余额
        #[ink(message)]
        pub fn allowance_of(&self, owner: AccountId, spender: AccountId) -> Result<Balance> {
            // *self.allowance.get(&(owner, spender)).unwrap_or(&0)
            let balance = Self::env().extension().allowance_of(owner, spender)?;
            Ok(balance)
        }
        ...
        pub fn allowance(&mut self, spender: AccountId, value: Balance) -> Result<()> {
            ...
            let _  = Self::env().extension().allowance(spender, value);
        ...
        pub fn transfer_from(
            ...
            let _ = Self::env().extension().transfer_from(from, to, value);
        ...
        fn transfer_helper(
            ...
            let _ = Self::env().extension().transfer_help(from, to, value);
